...
2. **API alignment (sub-step):** extend `POST /api/kzo/save_snapshot` response to meet **§4** of that contract (`created_at`, unified `failure` where applicable) — **separate commit** / micro-TASK if scope grows.
...
## TASK-2026-08B-011 — Stage **8B.1** Thin Client Adapter V1 (GAS)
...
| **Status** | `SHELL` — **implementation path registered**; **no** full coding unless explicitly tasked |
...
3. Call **`saveKzoSnapshotV1()`** / equivalent `UrlFetchApp` to **`/api/kzo/save_snapshot`**.
