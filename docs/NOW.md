...
**Next allowed step (governance):** **Gemini audit** (or equivalent) of **`docs/00-02_CALC_CONFIGURATOR/MODULE_01_CALCULATION_RECORD_VERSIONED_EDITOR_FLOW_V1.md`** — **DOC ONLY** until audit PASS; **no** Create Calculation Modal / API / GAS **implementation TASK** opened by this scope doc alone.
**Module 01 — Calculation record + editor flow (doctrine):** **Current step:** **Module 01 Calculation Record + Versioned Editor Flow V1** — **SCOPE DEFINITION / DOC ONLY** — **`docs/00-02_CALC_CONFIGURATOR/MODULE_01_CALCULATION_RECORD_VERSIONED_EDITOR_FLOW_V1.md`**. **Doctrine:** **calculation-record-first**; **item-level** equipment type; **initial version always `-00`**; revisions **`-01` / `-02` / …** without silent overwrite of committed versions. **Create Calculation Modal V1 implementation** — **not** activated by this entry; existing modal/spec remain **legacy boundary** until reconciled under audit.
...
**Module 01 Calculation:** **Calc record + versioned editor flow** — **scope defined** — **`docs/00-02_CALC_CONFIGURATOR/MODULE_01_CALCULATION_RECORD_VERSIONED_EDITOR_FLOW_V1.md`** (**DOC ONLY**, pending audit). **Create Calculation V1 code** — endpoint + modal (may diverge from doctrine until reconciliation); **technical spec** **`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CREATE_CALCULATION_TECHNICAL_SPEC.md`**; **implementation audit** **`docs/AUDITS/2026-05-07_MODULE_01_CREATE_CALCULATION_V1_IMPLEMENTATION.md`**. **KZO/product engineering engine not active**; planning **`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CREATE_CALCULATION_MODAL_V1_PLANNING.md`**; sidebar **`docs/ARCHITECTURE/EDS_POWER_MODULE_01_SIDEBAR_TECHNICAL_SPEC.md`**.
