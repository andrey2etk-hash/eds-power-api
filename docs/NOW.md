...
**EDS Power Terminal UI Shell:** **Current step:** **Terminal UI Shell Doctrine — DOC ONLY** — **`docs/ARCHITECTURE/EDS_POWER_TERMINAL_UI_SHELL_DOCTRINE.md`**. **Next allowed step:** **Gemini audit** of Terminal UI Shell Doctrine. **Not active:** GAS sidebar/modal implementation, registry schema change, calculation implementation.
**Module 01 Calculation:** **Paused until** Terminal UI Shell Doctrine is audited — baseline calc planning remains **`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CALC_SLICE_01_PLANNING.md`** (reference only; **do not** treat calc implementation as current step). **Not active:** calculation implementation (Python/GAS/DB/SQL/Render product code).
...
**Wider product/backend scope:** **Terminal UI Shell Doctrine** — **DOC ONLY** (`docs/ARCHITECTURE/EDS_POWER_TERMINAL_UI_SHELL_DOCTRINE.md`); **Module 01 Calc Slice 01** — planning doc retained (`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CALC_SLICE_01_PLANNING.md`); **calc implementation not active** until shell doctrine audit + explicit unlock.
