...
**Module 01 Sidebar:** **Current step:** **Module 01 Sidebar Static Context V1 — implemented / pending operator test** — closeout **`docs/AUDITS/2026-05-07_MODULE_01_SIDEBAR_STATIC_CONTEXT_V1_IMPLEMENTATION.md`**. **Backend:** **`GET /api/module01/sidebar/context`**. **GAS (bound sync names):** **`Module01Sidebar.gs`**, **`Module01SidebarHtml.html`** (repo: `gas/…`), **`AuthTransport.gs`**, **`EDSPowerCore.gs`** menu mapping. **Next allowed step:** operator GAS sync + live sidebar smoke **OR** registry `OPEN_MODULE_01_SIDEBAR` SQL planning (operator manual apply). **Not active:** Create Calculation modal, calculation endpoints, DB/SQL by Cursor.
...
**Wider product/backend scope:** **Module 01 Sidebar Static Context V1** — implemented (pending operator); **no** Cursor SQL/Render env. **Create Calculation** — **not** active.
