...
**EDS Power Terminal UX:** **Stabilization V1** — one **`EDS Power`** menu, registry items appended (deduped), **auto-open sidebar** when session valid, fallback **Відкрити бокову панель** — **`docs/AUDITS/2026-05-08_TERMINAL_UX_STABILIZATION_V1.md`** (**operator GAS sync required**).
...
**Module 01 — Create Calculation lane:** **Current step:** **Module 01 Create Calculation V1 implemented** — **`POST /api/module01/calculations/create`**, service + GAS modal + tests — **`docs/AUDITS/2026-05-07_MODULE_01_CREATE_CALCULATION_V1_IMPLEMENTATION.md`**. **GAS fixes:** modal bootstrap — public **`module01CreateCalculationBootstrap`** — **`docs/AUDITS/2026-05-07_MODULE_01_CREATE_CALCULATION_MODAL_BOOTSTRAP_FIX.md`**; submit success UI + **`module01CreateCalculationRememberActiveId`** (no `*_` from HTML) + duplicate guard — **`docs/AUDITS/2026-05-07_MODULE_01_CREATE_CALCULATION_MODAL_SUBMIT_UX_FIX.md`**. **Pending:** operator registry DML for **`MODULE01_CREATE_CALCULATION`** + live create smoke. **Next allowed step:** **operator registry DML apply + live create calculation test**. **Not active:** engineering / KZO calculation engine / BOM / snapshots. **Sidebar Static Context V1** — **`LIVE_PASS`** — **`docs/AUDITS/2026-05-07_MODULE_01_SIDEBAR_STATIC_CONTEXT_V1_LIVE_OPERATOR_TEST.md`**.
