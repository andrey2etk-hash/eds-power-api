...
**Module 01 — Create Calculation lane:** **Current step:** **Module 01 Create Calculation V1 implemented** — **`POST /api/module01/calculations/create`**, service + GAS modal + tests — **`docs/AUDITS/2026-05-07_MODULE_01_CREATE_CALCULATION_V1_IMPLEMENTATION.md`**. **GAS fix:** modal bootstrap must use public **`module01CreateCalculationBootstrap`** (not `*_`) — **`docs/AUDITS/2026-05-07_MODULE_01_CREATE_CALCULATION_MODAL_BOOTSTRAP_FIX.md`**. **Pending:** operator registry DML for **`MODULE01_CREATE_CALCULATION`** + live create smoke. **Next allowed step:** **operator registry DML apply + live create calculation test**. **Not active:** engineering / KZO calculation engine / BOM / snapshots. **Sidebar Static Context V1** — **`LIVE_PASS`** — **`docs/AUDITS/2026-05-07_MODULE_01_SIDEBAR_STATIC_CONTEXT_V1_LIVE_OPERATOR_TEST.md`**.
