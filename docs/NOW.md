...
**EDS Power Terminal governance patch:** **Current step:** **DOC ONLY** — **`docs/00_SYSTEM/02_GLOBAL_RULES.md`** (Render Thinking / GAS Thin UI; main.py Thin Router) + **`docs/ARCHITECTURE/EDS_POWER_GAS_DEPLOYMENT_AND_SYNC_DOCTRINE.md`**. **Context:** Terminal UI Shell Doctrine **Gemini = PASS / `ARCHITECTURE_LOCKED`**. **Next allowed step:** **Gemini audit** of Render Thinking / GAS deployment/sync / main.py thin router **governance patch**. **Not active:** Module 01 sidebar implementation, calculation implementation, GAS/backend refactors.
...
**Module 01 Calculation:** **Not active** — implementation (Python/GAS/DB/SQL/Render product code); baseline planning **`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CALC_SLICE_01_PLANNING.md`** remains reference; unlock only after explicit user direction + governance patch audit gate as applicable.
...
**Wider product/backend scope:** **Terminal governance patch** — DOC ONLY (`docs/00_SYSTEM/02_GLOBAL_RULES.md`, `docs/ARCHITECTURE/EDS_POWER_GAS_DEPLOYMENT_AND_SYNC_DOCTRINE.md`); **Terminal UI Shell** — **`ARCHITECTURE_LOCKED`**. **Module 01 Calc Slice 01** — planning doc retained (`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CALC_SLICE_01_PLANNING.md`); **calc / sidebar implementation not active**.
