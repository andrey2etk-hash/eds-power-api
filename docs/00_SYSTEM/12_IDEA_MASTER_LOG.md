...
- **Next:** **Create Calculation Technical Spec** — **`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CREATE_CALCULATION_TECHNICAL_SPEC.md`** — **schema confirmed**; **next:** implementation TASK; KZO/engine **not** active unless tasked. Sidebar static context V1 **live:** **`docs/AUDITS/2026-05-07_MODULE_01_SIDEBAR_STATIC_CONTEXT_V1_LIVE_OPERATOR_TEST.md`**.

### EDS_POWER_MODULE_01_SIDEBAR_PLANNING

Status:

- `SIDEBAR_DESIGN_LOCKED`
...
- **Next:** optional registry SQL for **`OPEN_MODULE_01_SIDEBAR`** (manual operator apply only); **Create Calculation** — **`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CREATE_CALCULATION_TECHNICAL_SPEC.md`** (**schema confirmed**; **implementation TASK** next) — **not** active: endpoint until TASK.
...
- `PLANNING_PASS_SCHEMA_CONFIRMED_TECH_SPEC`
...
- **Technical spec:** **`docs/ARCHITECTURE/EDS_POWER_MODULE_01_CREATE_CALCULATION_TECHNICAL_SPEC.md`** — **operator read-only schema CONFIRMED** (closeout in spec **§ Schema Confirmation Result**); **next:** implementation TASK (**not** auto-active).
...
- **GAS modal fixes (07.05.2026):** public **`module01CreateCalculationBootstrap`** — **`docs/AUDITS/2026-05-07_MODULE_01_CREATE_CALCULATION_MODAL_BOOTSTRAP_FIX.md`**; submit success UI + public **`module01CreateCalculationRememberActiveId`** — **`docs/AUDITS/2026-05-07_MODULE_01_CREATE_CALCULATION_MODAL_SUBMIT_UX_FIX.md`**.
...
- **Purpose:** Organize GAS deployment/sync before fleet file multiplication; repo = source of truth; recorded manual sync; optional groups A–D; future canonical deployment method TBD.
- Doctrine: **`docs/ARCHITECTURE/EDS_POWER_GAS_DEPLOYMENT_AND_SYNC_DOCTRINE.md`** — planning-era verdict **`GAS_DEPLOYMENT_AND_SYNC_DOCTRINE_READY_FOR_AUDIT`**; **governance patch PASS / `ARCHITECTURE_LOCKED`**.
- **User explicitly required** all thinking functions on **Render/backend**; GAS remains **UI shell**; **`main.py`** remains **route/wiring only** — see **`docs/00_SYSTEM/02_GLOBAL_RULES.md`**.
