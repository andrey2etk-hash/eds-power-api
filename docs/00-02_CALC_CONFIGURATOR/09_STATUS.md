...
- Stage **8A** — **code IMPLEMENTED / live verification PENDING** — **`calculation_snapshots`** DDL **held** (**8A.0.2** **`IDEA-0020`**, **`LEGACY_REMOTE_BASELINE.md`**) до baseline import; **`POST /api/kzo/save_snapshot`**; **8A.0.1** naming **`IDEA-0019`**; **IDEA-0017** **`ACTIVE`** (**`PENDING_SUPABASE_VERIFICATION`**); live gate **`docs/AUDITS/2026-04-29_STAGE_8A_SUPABASE_LIVE_VERIFICATION_GATE.md`**

На цьому рівні виконано:

Структурна топологія та **Stages 7A / 7B** узгоджені з аудитами: **Stage 7B CLOSED**. **Stage 8A** persistence: **код implemented** у репозиторії; **`IDEA-0017`** **`ACTIVE`** до **live PASS** Supabase (**`docs/AUDITS/2026-04-29_STAGE_8A_SUPABASE_LIVE_VERIFICATION_GATE.md`**). Frozen **V1 INSERT** only — retrieval / analytics — поза Stage 8A verification.
