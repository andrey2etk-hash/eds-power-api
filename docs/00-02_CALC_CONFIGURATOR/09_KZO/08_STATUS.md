...
Stage 8A = **`FIRST_PERSISTENCE_READY_NON_PROD`** (**2026-04-30**): активна **`20260429120000_calculation_snapshots_v1.sql`**, локальний **`supabase db reset`**, legacy + **`calculation_snapshots`** верифіковані (аудит **8A.1**). **IDEA-0017** **`ACTIVE`** до live gate. **Немає** prod apply в **8A.1**.
...
**Stage 8A:** **`FIRST_PERSISTENCE_READY_NON_PROD`** — **IDEA-0017** **`ACTIVE`** до live gate; **IDEA-0022** **`IMPLEMENTED`**; retrieval/history UI — окремий TASK.
...
**Stage 7B — formally closed** (Gemini **`SAFE TO PROCEED TO STAGE 8A`**). **`KZO_MVP_SNAPSHOT_V1`** is the **only** approved persistence-shape baseline until a new snapshot **`IDEA`** — **IDEA-0016** **`IMPLEMENTED`**. **Stage 8A** — міграція **`calculation_snapshots`** у **`supabase/migrations/`**; локальний **`db reset`** **PASS** (**8A.1**). **IDEA-0017** **`ACTIVE`** (**`PENDING_SUPABASE_VERIFICATION`**) до **live PASS**. **IDEA-0022** **`IMPLEMENTED`**.
